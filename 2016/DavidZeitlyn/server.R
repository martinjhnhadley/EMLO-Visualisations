...
component_names_for_ui <-
  setNames(as.list(1:length(decomposed_igraph)), c("Main Lineage", paste(
    "Minor Lineage Number", 2:length(decomposed_igraph)
  )))
...
shinyServer(function(input, output, session) {
  ...
  source("shared_graph_functions.R", local = TRUE)$value
  ...
  output$main_component_or_subcomponents_UI <- renderUI({
    selectInput("main_component_or_subcomponents",
                label = "Show main lineage or minor lineages",
