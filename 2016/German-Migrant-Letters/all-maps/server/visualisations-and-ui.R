...
output$show_routes_UI <- renderUI({
  checkboxInput(inputID = "show_routes", label = "Show letter routes?", value = TRUE)
})

output$countries_to_show <- renderUI({
  if(is.null(input$show_routes)){
    return()
  }
  
  if(input$show_routes){
    selectInput(inputId = "countries_to_show", choice = c("World","Europe","USA"), label = "Countries to Show", selected = "World")
  }
})
