...
shinyServer(function(input, output, session) {
  ...
  output$department_app_title <-
    renderUI(h1(paste(url_selected_department(), "Profile")))
  ...
  output$department_app_collapsile_info <- renderUI(fluidRow(
    column(paste0(
      ...
      sum(institution_nodes$department == url_selected_department())
    ),
    width = 6),
    ...
    column(paste0(
      "Collaborations in Department:", ecount(induced_subgraph(
        institution_igraph,
        institution_nodes[institution_nodes$department == url_selected_department(),
                          "name"]
      ))
    ),
    width = 6),
  ...
  output$people_directory_DT <- DT::renderDataTable({
    institution_nodes[institution_nodes$department == url_selected_department(), c("name", "institution", "department")]
  }, rownames = FALSE,
  # filter = FALSE,
  escape = FALSE,
  ...
  output$people_directory_UI <- renderUI(fluidPage(
    paste("These are the people in the ", url_selected_department()),
                  ...
                  switch(
                    ...
                    "within whole network" = {
                      graph.union(make_ego_graph(institution_igraph,
                                                 order = 2,
                                                 nodes = institution_nodes[institution_nodes$department == url_selected_department(), "name"])) %>% igraph_deduplicate_vertex_attr()
                    },
