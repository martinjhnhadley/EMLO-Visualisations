...
weekheatmap_data <- eventReactive(
  c(
  ...
  ),
  {
    hc_data <- weekly_patrols_and_gunshots %>%
      filter(
        year %in% input$weekheatmap_years_selected &
    ...
    hc_data <- hc_data %>%
      ...
      mutate(
        patrol.range = cut(
          ...
          labels = paste0(format(seq(0, 110000, 10000)), "-", format(seq(
            10000, 120000, 10000
          ))),
          include.lowest = TRUE
        )
...
output$weekheatmap_hc <- renderHighchart({
  ...
  hchart(
    weekheatmap_data,
    "heatmap",
    hcaes(x = rainfall.range, y = patrol.range, value = mean.shots)
  ) %>%
    ...
    hc_credits(text = 'Data from WildCRU',
               enabled = TRUE,
